...
public:
	...
	void remove(Element element_to_be_removed);
	bool update(Element element_to_be_updated);
	bool is_in_list(Element element_searched);
...
template <class Element>
void DynamicVector<Element>::remove(Element element_to_be_removed)
{	
	
	
	for (int i = 0; i < this->lenght; i++)
	{
		if (this->elements[i] == element_to_be_removed)
		{
...
template <class Element>
bool DynamicVector<Element>::update(Element element_to_be_updated)
{
	

	for (int i = 0; i < this->lenght; i++)
	{
		if (this->elements[i]==element_to_be_updated)
		{
...
bool DynamicVector<Element>::is_in_list(Element element_searched)
{
	for (int i = 0; i < this->lenght; i++)
	{
		if (this->elements[i] == element_searched)
		{
			return true;
